...
_USB_bReadOutEP:
    ...
    MOV     A, reg[X+USB_EP0CNT]        ; Get the Real count
    AND     A, BYTE_CNT_BITMASK         ; Mask off the count bits
    SUB     A, BYTE_CNT_CHKSUM         ; Count includes the two byte checksum    
    MOV     [USB_APICount], A           ; Save the Real count of bytes sent by the host
...
.CountDetermined:
    ...
    MOV     A, [USB_APITemp]            ; Check the Count
    ...
    MOV     [USB_APITemp+1], A          ; Save the LSB of the pointer
...
.loop:
    ...
    MVI     [USB_APITemp+1], A          ; save it in data array
    INC     X                           ; increment the pointer
    DEC     [USB_APITemp]               ; decrement the counter
    JNZ     .loop                       ; wait for count to zero out


    MOV     A, [USB_APICount]           ; Get the Real count LSB
.done:
.exit:
    RET
.ENDSECTION
