...
USB_EP0_UPD_MODE_EXIT:
    ...
    MOV     [USB_t0], A                    ; store EP0_CNT reg value for later
    MOV     A, reg[USB_EP0MODE]            ; unlock registers  
    AND     A, 80h                                      ; Check if SETUP bit is set
    JNZ     USB_EP0_ISR_EXIT                ;          then EXIT from the ISR	
    MOV     A, [USB_t0]                    ; restore EP0_CNT reg value
    MOV     reg[USB_EP0CNT], A             ; Update the count register
    MOV     A, reg[USB_EP0CNT]             ; writing check
    CMP     A, [USB_t0]                    ; if writing was not succesful,
    JNZ     USB_EP0_ISR_EXIT               ;         then abort the ISR    

    MOV     [USB_t0], X                     ; store EP0_MODE reg value for later
.ReWriteMODE:
    MOV     A, reg[USB_EP0MODE]             ; unblock registers    
    AND     A, 80h                                      ; Check if SETUP bit is set
    JNZ     USB_EP0_ISR_EXIT                ;          then EXIT from the ISR
    
    MOV     A, [USB_t0]                     ; restore the new MODE
    MOV     reg[USB_EP0MODE], A             ; Update the mode regiter
    
    MOV     A, reg[USB_EP0MODE]             ; writing check
    AND     A,  0Fh                                     ; clear 7-4 bits
    CMP     A, [USB_t0]                     ; if writing was succesful,
    JZ      USB_EP0_ISR_EXIT                ;          then EXIT from the ISR
...
; Check the byte count and validity.  All SETUP are 8 bytes and 0 toggle
    MOV     A, reg[USB_EP0MODE]    ; unlock registers
    MOV     reg[USB_EP0MODE], A ; clear setup bit 
    MOV     A, reg[USB_EP0MODE]    ; (re)read register
    AND     A, 80h                              ; Check if SETUP bit is set
    JNZ     USB_EP0_ISR_EXIT       ; if SETUP bit set -> exit without modifying the mode    
